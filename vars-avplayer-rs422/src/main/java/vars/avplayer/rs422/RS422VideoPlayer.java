...
public class RS422VideoPlayer implements VideoPlayer<RS422State, RS422Error> {
    ...
    private final ImageCaptureService imageCaptureService = new AVFImageCaptureServiceImpl();
    ...
    public Optional<Tuple2<VideoArchive, VideoController<RS422State, RS422Error>>> openVideoArchive(ToolBelt toolBelt, Object... args) {
        ...
        VideoParams videoParams = new VideoParams(serialPort, platformName, sequenceNumber, tapeNumber, isHD);
        return openVideoArchive(toolBelt, videoParams);
    }

    public Optional<Tuple2<VideoArchive, VideoController<RS422State, RS422Error>>> openVideoArchive(ToolBelt toolBelt, VideoParams videoParams) {
        ...
        if (videoIORef.get() != null) {
            ...
            if (!connectionID.equalsIgnoreCase(videoParams.getSerialPortName())) {
                videoIO.close();
                videoIORef.set(null);
            }
        }
        ...
        if (videoIORef.get() == null) {
            RS422VideoIO videoIO = RXTXVideoIO.open(videoParams.getSerialPortName());
            videoIORef.set(videoIO);
        }

        return Optional.of(new Tuple2<>(videoArchive,
                new VideoController<>(imageCaptureService, videoIORef.get())));
    }


    @Override
    public VideoPlayerDialogUI<RS422State, RS422Error> getConnectionDialog(ToolBelt toolBelt) {
        return null;
    }
