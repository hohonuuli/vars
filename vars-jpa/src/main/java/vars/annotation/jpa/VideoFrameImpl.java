...
@NamedQueries({ @NamedQuery(name = "VideoFrame.findById", query = "SELECT v FROM VideoFrame v WHERE v.id = :id") ,
                ...
                @NamedQuery(name = "VideoFrame.findByTimeCodeAndVideoArchiveName",
                            query = "SELECT v FROM VideoFrame v WHERE v.timecode = :timecode AND v.videoArchive.name = :videoArchiveName")})
