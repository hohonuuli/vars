...
class GConcept implements Serializable, Concept, JPAEntity {
    ...
    @ManyToOne(optional = true, targetEntity = GConcept.class, cascade = [CascadeType.MERGE])
    @JoinColumn(name = "ParentConceptID_FK")
    Concept parentConcept

    @OneToMany(targetEntity = GConcept.class,
            mappedBy = "parentConcept",
            fetch = FetchType.LAZY,
            cascade = [CascadeType.ALL])
    Set<Concept> childConcepts

    @OneToMany(targetEntity = GConceptName.class,
            mappedBy = "concept",
    ...
    void addConceptName(ConceptName conceptName) {
        if (getConceptNames().find { ConceptName cn -> cn.name.equals(conceptName.name) }) {
