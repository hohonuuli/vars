...
if (args.size() < 6) {
    println("""
    | Usage:
    |   gsh load_tripod <url> <sequence number> <longitude> <latitude> <lens to seafloor distance> <angle of inclination of camera>
    |
    | Args:
...
def latitude = Double.parseDouble([3])
def lensToSeaFloorDistance = Double.parseDouble(args[4])
def angleOfInclination = Double.parseDouble(args[5])

def loader = new TripodLoader()
loader.load(remoteDirectoryUrl, platform, sequenceNumber, longitude, latitude, lensToSeaFloorDistance, angleOfInclination)
